...
class PatientContactForm(ModelForm):
    ...
    def clean(self):
            ...
            if has_appointment == models.PatientContact.PTSHOW_YES:
                ...
                if pt_went == models.PatientContact.PTSHOW_NO:
                    if not cleaned_data.get("no_show_reason"):
                        self.add_error(
                            "no_show_reason", "Why didn't the patient go to the appointment?")
                ...
                if cleaned_data.get("appointment_location"):
                    self.add_error(
                        "appointment_location", _("Cannot speicfy a location if patient has not made appointment."))
                ...
                if not cleaned_data.get("has_appointment"):
                    self.add_error(
                        "has_appointment", _("Need to specify if patient ") +
                        _("has appointment"))
        ...
        if self.SUCCESSFUL_REFERRAL in self.data:
            if pt_went != models.PatientContact.PTSHOW_YES:
                self.add_error(
                    "pt_showed", _("Cannot submit a successful ") +
                    _("referral if the patient did not show up for") +
                    _(" an appointment"))
        ...
        if self.REQUEST_FOLLOWUP in self.data:
            if pt_went == models.PatientContact.PTSHOW_YES:
                self.add_error(
                    "pt_showed", _("Cannot give up on a ") +
        ...
        if self.UNSUCCESSFUL_REFERRAL in self.data:
            if pt_went == models.PatientContact.PTSHOW_YES:
                self.add_error(
                    "pt_showed", _("Cannot request a new referral ") +
                    _("follow up on a successful referral"))
